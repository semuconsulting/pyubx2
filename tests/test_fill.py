...
class FillTest(unittest.TestCase):
    ...
    def testFill_RXMPMPSETV0(self):  #  test RXM-PMP GET constructor with version=0 keyword
        ...
        res = UBXMessage('RXM', 'RXM-PMP', GET, version=0)
        self.assertEqual(str(res), EXPECTED_RESULT)

    def testFill_RXMRLMSETV1(self):  #  test RXM-RLM GET constructor with version=1 keyword
        ...
        res = UBXMessage('RXM', 'RXM-PMP', GET, version=1)
        self.assertEqual(str(res), EXPECTED_RESULT)
    ...
    def testFill_ESFMEASSET(self):  #  test ESF_MEAS GET constructor with calibTtagValid=False (flags=0) keyword
        EXPECTED_RESULT = "<UBX(ESF-MEAS, timeTag=0, flags=b'\\x00\\x00', id=0)>"
        res = UBXMessage('ESF', 'ESF-MEAS', GET, timeTag=0, flags=b'\x00\x00')
        self.assertEqual(str(res), EXPECTED_RESULT)

    def testFill_ESFMEASSETCT(self):  #  test ESF_MEAS GET constructor with calibTtagValid=True (flags=8) keyword
        EXPECTED_RESULT = "<UBX(ESF-MEAS, timeTag=0, flags=b'\\x08\\x08', id=0, calibTtag=0)>"
